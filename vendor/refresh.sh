#!/usr/bin/env bash
#
# refresh.sh — regenerate the vendored OneTerm forks of `vte` and `alacritty_terminal`
...
# fetched from the network (crates.io / GitHub). Requires: bash, patch, diff, tar;
# and git + curl only for the network fallback.

set -euo pipefail

# `-b` makes --check insensitive to Git's platform line-ending conversion while still
# detecting non-whitespace source changes.
