...
pub fn new(config: &Options, window_size: WindowSize) -> Result<Pty> {
    ...
    // myTerm2: CREATE_NEW_PROCESS_GROUP — shell gets own process group ID.
    // GenerateConsoleCtrlEvent(CTRL_C_EVENT, shell_pid) sends signal chỉ đến
    // shell's group (shell + children), KHÔNG gửi đến myTerm2 (group 0).
    // → Ctrl+C không exit myTerm2, chỉ interrupt child process.
