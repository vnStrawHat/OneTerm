...
This makes the assumption checkable: the full ignore list is diffed against
`scripts/ignored-tests.txt`, so **any** new ignored test anywhere fails the gate
until someone records it and decides whether it belongs in the elevation run.
Same pattern as `oneterm_actions::elevated_policy` — an exhaustive table where
unclassified fails the build — applied to the ignore list.
...
def main() -> int:
    ...
    if arguments.write:
        header = (
            ...
            "#\n"
            "# An ignored test runs in no gate by default. This file is diffed by\n"
        ...
        print(f"check-ignored-tests: recorded {len(observed)} ignored tests")
        return 0
