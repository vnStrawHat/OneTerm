...
| `verify-dependency-graph.py` | Enforce `dependency-graph-policy.json` (workspace members, internal edges, backend/feature rules), that every crate inherits `[workspace.package] version`, that **no** crate is publishable (owner ruling 2026-09-15: nothing goes to crates.io), that no tracked path exceeds 150 characters (a longer one breaks a git dependency's checkout on Windows), and, given a `cargo package --list` on stdin via `--package-list -`, that the `oneterm-vt` package carries `README.md`, `CHANGELOG.md`, `LICENSE`, `NOTICE` and `examples/headless.rs` and reaches nothing outside `crates/vt`. | CI |
| `vt-public-api.py` | Enumerate `oneterm-vt`'s public API from its rustdoc output and diff it against the committed snapshot, so a change to the public surface is a line in a review. There are two snapshots, `crates/vt/public-api.windows.txt` and `crates/vt/public-api.unix.txt`, because the `pty` module's surface is cfg-dependent and `cargo doc` renders only the host's half; the script picks by host and says which it used. `--update` regenerates, `--check` gates, `--check-nameable` fails on a public signature naming a type an embedder cannot write down, `--no-doc` reuses an existing `target/doc`, and `--diff-platforms` needs no rustdoc at all. **`--update`, `--check` and `--check-nameable` read whatever `target/doc` holds**, so the last doc build before them must be the `--all-features` one -- the committed snapshots are the all-features surface, and after a bare `cargo doc -p oneterm-vt --no-deps` the `regex` items are missing and `--check` fails. Both CI entry points already order it that way. | CI (`vt-package` job) |
| `vt-docs.ps1` / `vt-docs.sh` | Render `oneterm-vt`'s API reference and its embedder's guide (`RUSTDOCFLAGS='-D warnings' cargo doc -p oneterm-vt --no-deps --all-features`) and print the guide's index path. Adds no CI step: the `vt-package` job and `ci-local` already run that exact invocation, and this is the one-command local twin. Run it **after** `vt-public-api.py --check --no-doc`, which reads whatever `target/doc` was left behind. | local (same command as CI) |
