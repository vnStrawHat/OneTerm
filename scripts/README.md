...
| `vt-public-api.py` | Enumerate `oneterm-vt`'s public API from its rustdoc output and diff it against the committed snapshot, so a change to the public surface is a line in a review. There are two snapshots, `crates/vt/public-api.windows.txt` and `crates/vt/public-api.unix.txt`, because the `pty` module's surface is cfg-dependent and `cargo doc` renders only the host's half; the script picks by host and says which it used. `--update` regenerates, `--check` gates, `--no-doc` reuses an existing `target/doc`, and `--diff-platforms` fails if the two snapshots disagree anywhere outside `oneterm_vt::pty`. | CI (`vt-package` job) |
