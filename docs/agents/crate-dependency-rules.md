...
L1  actions · settings · update   (gpui action structs / config load-save / GitHub Releases updater)
    │
L0  core · terminal · completion · highlight · vt   (domain + engines + leaf; core = pure domain)
```
...
`highlight` and `vt` are leaves. `vt` (`oneterm-vt`, IN-0029) is OneTerm's own VT
engine: it depends on **no** OneTerm crate at all, which is stricter than R2
requires, and nothing depends on it yet — `crates/terminal` still owns
`alacritty_terminal` until the IN-0029 migration packets land. `crates/tools` (`oneterm-tools`, developer diagnostics: DOOM-fire
...
| **R6** | **`core` is pure domain.** No `gpui`, no `gpui-component`, no `alacritty_terminal`, no `oneterm-vt`. Types + traits (`AppError`, `SftpBackend`, `SshConfig`, `LocalShellConfig`) only. | The domain must not pull in UI or a specific terminal engine. | `cargo tree -p oneterm-core -e normal` shows no `gpui*`, no `alacritty_terminal` and no `oneterm-vt`. |
| **R7** | **The engines are gpui-free.** `terminal` is alacritty-coupled but MUST NOT depend on `gpui` / `gpui-component`; `completion`, `highlight` and `vt` depend on neither `gpui` nor `alacritty_terminal`, and `vt` additionally depends on no OneTerm crate. | The engines are reusable and unit-testable without a UI. | `cargo tree -p oneterm-terminal -e normal`, `-p oneterm-completion`, `-p oneterm-highlight`, `-p oneterm-vt` show no `gpui*`. |
