...
- **Agent forwarding** is a per-session switch in the session dialog ("Forward the SSH agent to the remote host"), off by default and independent of the authentication method (DEC-0011). When on, the connection requests `auth-agent-req@openssh.com` on the session channel right after it opens; a refusal (`AllowAgentForwarding no`) is one warning toast and the shell still opens. Every agent channel the server then opens is bridged to a fresh connection to the local agent (`crates/ssh/src/agent.rs::spawn_agent_bridge`), so `ssh` and `git` on the remote host can sign with the local keys while the tab is open. When the switch is off, an agent channel the server opens anyway is closed unanswered: a server cannot obtain agent access the user did not grant. Jump hops never see agent channels. Security note: while forwarding is on, root on the remote host can use every key the local agent holds; keep it per session and prefer `ssh-add -c` for confirmation prompts.
