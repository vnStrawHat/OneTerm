...
and the same pair for `snapshot_update`. No production line changed: `Screen::integrity_lo`,
`Screen::assert_integrity`, `TerminalGrid::assert_integrity` and the `vt-paranoid` feature are
untouched, and so is the sibling bench in the same file.
...
The ratio never left 0.82x-1.02x, a factor of 20 inside its bound. A ratio slightly *below* 1 —
the full-history probe costing marginally less than the near-empty one — is the two probes being
the same measurement twice, plus the second one paying a cold cache; it is noise, not a finding,
and it is the shape R-28 predicts.
...
Complete. The product is unchanged, the three owning docs are reconciled, and nothing is owed to a
next owner.
