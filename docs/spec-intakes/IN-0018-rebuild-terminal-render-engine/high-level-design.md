...
| `prepaint` | `GridGeometry` from bounds; resize session if `(rows, cols)` changed; `frame.snapshot`; upload new Sixel images into the `GraphicStore`; `plan_cache.update`; overlays; cursor; gutter labels for visible rows | `insert_hitbox(bounds, Normal)` |
...
| `force_width` numbers glyphs, not cells: after a ligature every later glyph lands one cell left | shifted text with `calt` on | the painter anchors each glyph at the cell of its byte index (`RowPlan::cell_starts`, `CellAnchor`; IN-0027) |
