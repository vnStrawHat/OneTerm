...
| Backend | `oneterm-local-shell` | Local PTY session implementation (owns the poll loop over `oneterm-pty`) | `crates/local-shell/src/lib.rs`, `crates/local-shell/src/session_terminal.rs`, `crates/local-shell/src/event_loop.rs` |
