...
| Terminal engine | `oneterm-vt` | OneTerm's own VT engine (`IN-0029`, `DEC-0014`): bytes to grid, damage and snapshot state, reflow, selection, graphics. A leaf — no OneTerm dependency, no UI framework, no lock, no interior mutability | `crates/vt/src/lib.rs`, `crates/vt/src/terminal/`, `crates/vt/src/grid/`, `crates/vt/src/snapshot/`, `crates/vt/src/parser/` |
...
| PTY transport | `oneterm_vt::pty` (default-on `pty` feature) | Pseudo-console transport with no grid: ConPTY (bundled `conpty.dll` first, `kernel32` as the fallback) or `openpty`, behind `PseudoConsole` / `EventedPty` / `OnResize` | `crates/vt/src/pty/mod.rs`, `crates/vt/src/pty/windows.rs`, `crates/vt/src/pty/unix.rs` |
...
| Backend | `oneterm-local-shell` | Local PTY session implementation (owns the poll loop over `oneterm_vt::pty`) | `crates/local-shell/src/lib.rs`, `crates/local-shell/src/session_terminal.rs`, `crates/local-shell/src/event_loop.rs` |
