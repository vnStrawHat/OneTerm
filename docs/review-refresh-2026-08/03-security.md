...
- [ ] **[Medium] SEC-03 — OSC 8 display/target mismatch defence is dead code.**
...
- [ ] **[Medium] SEC-04 — Per-agent dedup map is unbounded and terminal-controlled.**
...
- [ ] **[Low] SEC-07 — Control-character filters allow `\n`/`\r`/`\t` in URLs and cwd; BiDi isolates not
...
- [ ] **[Low] SEC-08 — Clipboard-read policy applied inconsistently.** `local-shell/src/listener.rs:344-346`
...
- [ ] **[Low] SEC-09 — Attached-value secrets leak into completion history** (`-pSECRET`, `-uuser:pass`,
  `crates/completion/src/redact.rs:64`). *Fix:* attached-form detection for vocabulary flags.

- [ ] **[Low] SEC-10 — Copy-on-select is unconditional.** `terminal-view/src/handlers/mouse.rs:111-117`
...
- [ ] **[Low] SEC-16 — Unbounded reads of `/etc/passwd`/`/etc/group` from an untrusted server**
...
- [ ] **[Medium] SEC-18 — Disabling certificate verification silently removes all integrity.**
...
- [ ] **[Low] SEC-19 — Proxy URL (may embed credentials) echoed into logs on parse failure**
  (`update/src/github.rs` `invalid update proxy URL '{proxy_url}'`). *Fix:* redact userinfo.

- [ ] **[Low] SEC-20 — No size cap on download/extraction** (`github.rs download_to_file`, `archive.rs`).
  *Fix:* cap by `asset_size` and a sane extracted-bytes limit.

- [ ] **[Low] SEC-21 — Zip entries: no symlink/unix-mode handling** (`archive.rs:149-177`; tar rejects links,
  zip does not). *Fix:* same guard for defence in depth.

- [ ] **[Low] SEC-22 — `asset_url` not restricted to `https://`; `parse_release_version(CURRENT_VERSION)`
...
- [ ] **[Low] SEC-23 — Updater target repository inferred at build time from `git remote`**
...
- [ ] **[Low] SEC-24 — Crash reports: only `$HOME` is redacted; files use default permissions.**
...
- [ ] **[Low] SEC-25 — Legacy crash-report import follows symlinks** (`crash_report.rs:116-134` `fs::read` +
