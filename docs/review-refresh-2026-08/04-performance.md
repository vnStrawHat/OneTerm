...
- [ ] **[High] PERF-01 — `build_terminal_theme` re-parses JSON every frame.**
...
- [ ] **[High] PERF-02 — Gutter re-shaped every frame.** `element/paint.rs:254-305` calls `shape_line` for
...
- [ ] **[High] PERF-03 — `terminal_info()` scans the whole viewport under the `Term` lock and is called 3× per
...
- [ ] **[Medium] PERF-05 — Per-frame clones/allocs in `render`:** `settings.color_overrides.clone()`
...
- [ ] **[Medium] PERF-06 — Completion re-clones the whole grid on cursor move.**
...
- [ ] **[Medium] PERF-09 — Shaping allocates `SharedString::from(run.text.clone())` per newly-laid-out run**
...
- [ ] **[Low] PERF-12 — `line_times: VecDeque<String>` stores an 8-char heap `String` per scrollback line**
...
- [ ] **[Low] PERF-13 — `cell_style` clones `Font` per non-blank cell; `can_append` deep-compares `Font` per
...
- [ ] **[Low] PERF-15 — URL hover query on every mouse move** (`handlers/mouse.rs:248` → `url.rs:37-44`, 11-line
  grid clone per move). *Fix:* skip when `row/col` unchanged.

- [ ] **[Low] PERF-16 — `cursor_bounds()` clones the whole grid to read the cursor** (`crates/terminal/src/model.rs:374`,
  IME path). *Fix:* `renderable_content()` cursor + `display_offset()` under one lock.

- [ ] **[Low] PERF-17 — Wheel takes three separate locks** (`model.rs:339-343`). *Fix:* one lock scope.
...
- [ ] **[Medium] PERF-23 — Highlight scanner rebuilds the same string three times per line.**
...
- [ ] **[Medium] PERF-26 — `System::new_all()` at every startup on the main thread.**
...
- [ ] **[Medium] PERF-27 — Persistence work on the UI thread at startup.**
...
- [ ] **[Low] PERF-28 — `ResourceIndicator` refresh on the UI thread every 2 s with a heavier-than-needed kind**
...
- [ ] **[Low] PERF-29 — Polling widgets notify even when unchanged.** `net_speed.rs:209` `cx.notify()`
