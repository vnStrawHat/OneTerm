...
| [`spec-intakes/IN-0039-vt-gaps-and-publish/high-level-design.md`](spec-intakes/IN-0039-vt-gaps-and-publish/high-level-design.md) + its `low-level-design/` | The four gaps an outside evaluation of `oneterm-vt` found: unnameable public return types, the kitty keyboard encoder, `DECRQCRA`/`DECRQSS`/`XTGETTCAP` plus an `esctest` report, and published performance evidence. Publishing to crates.io is **not** in scope -- IN-0038 Open Decision 2 stands | **proposed owning design**, extending IN-0038 (records only; not yet implemented) |
| [`spec-intakes/IN-0040-view-key-release-repeat/high-level-design.md`](spec-intakes/IN-0040-view-key-release-repeat/high-level-design.md) + its `low-level-design/` | The terminal view delivers key **release** and **repeat** to the engine, so the kitty `REPORT_EVENT_TYPES` flag OneTerm already negotiates is honoured inside OneTerm. Follows IN-0039's `US-0105`, which shipped the encoder and left the view as the remaining half; changes no file under `crates/vt` | **proposed owning design** for the view's keyboard path (records only; not yet implemented) |
