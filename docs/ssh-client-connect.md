...
| The server presents an **OpenSSH host certificate** instead of a bare key | `AppError::HostKeyUnknown` with the fingerprint prefixed `cert:` — refused, and not approvable from the UI, because approving would write a certificate's fingerprint into a `known_hosts` file that records bare keys. OneTerm has no certificate-authority trust store, so it cannot tell a valid certificate from an attacker's. This cannot arise against a conforming server: OneTerm advertises no `*-cert-v01@openssh.com` host-key algorithm (russh's `Preferred::host_key_certificates` is empty by default and OneTerm builds no custom `Preferred`), so a server has no way to offer one. The refusal exists so the fail-closed rule holds even against a non-conforming server. |
