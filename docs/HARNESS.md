...
| A distinct new outcome or independently acceptable slice surfaces within an intake (including scope discovered mid-work) | Create a new `US`/`BUG` for it instead of expanding an existing packet. Keep each packet to one coherent, independently verifiable outcome; do not fold unrelated new work into a done or in-flight story, and do not split one outcome into trivially small packets. |
...
Scope one work packet to one outcome. A well-sized `US`/`BUG` states a single coherent outcome, is acceptable and verifiable on its own, and lists the packets it depends on. An intake normally owns several such packets rather than one packet that absorbs everything: prefer a new packet for each distinct or newly discovered outcome, and reserve extending an existing packet for steps that serve its stated outcome. Balance this against over-splitting — do not create separate packets for changes that are designed, verified, and accepted together, or for trivial edits that belong to a larger outcome. When unsure, ask whether the work could be accepted independently: if yes, it is its own packet; if no, it belongs to an existing one.
...
Generated Intakes use a gap-free document sequence independent of DB-only classifications and live at `docs/spec-intakes/IN-NNNN-slug/IN-NNNN.md`. Keep Intake titles short so the derived folder and file slugs stay concise; the generator caps each slug and truncates at a word boundary, but a focused title (a few words naming the outcome) reads better than a long clause. `intake create` supports every request type so standalone bugs and maintenance can own a folder; `intake add` remains the DB-only classification path. Every `intake create` also generates the intake's one mandatory High-Level Design at `<intake-folder>/high-level-design.md` and records its path. When the intake touches any user-facing surface, the HLD's UI Wireframe section is required: sketch each affected view as an ASCII wireframe; mark it `N/A` only when there is genuinely no UI change. Detail (Low-Level) Design is optional and split by concern under `<intake-folder>/low-level-design/<concern>.md` via `design lld`; it is **required for the high-risk lane**, where `story create` is blocked until at least one detail design file exists. Work IDs use `US-NNNN` or `BUG-NNNN`; `story create` requires their owning `IN-NNNN` and keeps the packet inside that Intake folder, including when `--doc` is supplied. Decision IDs use `DEC-NNNN` and remain under `docs/decisions/`. The CLI updates only marked Harness status/proof task-list blocks (or appends them to older/custom templates); authored Acceptance and Plan checklists remain human/agent-owned. Keep templates useful and concise.
