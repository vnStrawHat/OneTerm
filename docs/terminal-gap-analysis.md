# Phân tích Gap: Terminal OneTerm vs Zed Terminal

> Ngày tạo: 2025-07-14
> Tham chiếu Zed: `zed-industries/zed` @ commit `20a3f770` (main branch)
> Tham chiếu OneTerm: `crates/core/src/terminal/` + `crates/ui/src/views/terminal/`

---

## Tổng quan

| Khía cạnh | Zed | OneTerm | Mức độ hoàn thành |
|---|---|---|---|
...
| ✅/❌ | Gap | Zed có | OneTerm có | Mô tả / Ví dụ |
|---|---|---|---|---|
...
| ❌ | **Image protocol (Sixel/iTerm2/Kitty)** | Có — hỗ trợ iTerm2 inline image qua `repl` crate | Không | Zed: `cat image.png` qua `imgcat` hiện ảnh inline. OneTerm: chỉ thấy escape sequences rác. |
...
| ✅ | **Right-click context menu** | Có (richer) | Có (basic: Copy/Paste/Select All/Clear) | Zed thêm: New Terminal, Inline Assist, Close Tab. OneTerm: 4 items cơ bản. |
| ❌ | **Paste image as Ctrl+V** | Có — phát hiện `ClipboardEntry::Image` → gửi Ctrl+V | Không | Zed: copy ảnh → paste vào terminal → gửi raw Ctrl+V. OneTerm: paste ảnh → không làm gì. |
| ❌ | **Drag-and-drop file paths** | Có — `ExternalPaths` → quote path → write to PTY | Không | Zed: kéo file từ Finder vào terminal → path tự động quote. OneTerm: không nhận file kéo vào. **Ví dụ**: Kéo `main.rs` vào terminal → Zed auto `/path/to/main.rs`. |
...
| ✅/❌ | Gap | Zed có | OneTerm có | Mô tả / Ví dụ |
|---|---|---|---|---|
| ❌ | **In-terminal search** | Có — `SearchableItem` trait, `SearchEvent`, `SearchQuery` | Không | Zed: Cmd+F → search bar hiện, gõ "error" → highlight tất cả match. OneTerm: không search được. **Ví dụ**: `cargo build` output 1000 dòng → Cmd+F "warning" → Zed highlight tất cả. OneTerm: phải đọc bằng mắt. |
...
| ❌ | **Path-like hyperlink (file:line)** | Có — `hover_path_like_target`, `open_path_like_target`, regex cho `file.rs:42` | Không | Zed: hover `src/main.rs:42` → tooltip hiện path, Ctrl+click mở file tại line 42. OneTerm: chỉ URL. **Ví dụ**: Compiler output `error at src/main.rs:42:10` → Zed: click mở editor. OneTerm: text thường. |
| ❌ | **Hover tooltip** | Có — `HoverTarget { tooltip, hovered_word }` | Không | Zed: hover URL/path → tooltip hiện full URL/path. OneTerm: không có tooltip. |
| ❌ | **Hover underline** | Có — underline URL khi hover | Không | Zed: hover URL → URL gạch dưới. OneTerm: URL không có visual feedback khi hover. |
| ❌ | **File path detection (relative)** | Có — detect relative path + resolve against cwd | Không | Zed: `./src/main.rs` → Ctrl+click mở. OneTerm: không detect. |
...
| ⚡ | **Shell environment detection** | Có — `ProjectEnvironment`, `capture_unix/windows`, `zed --printenv` | Không | Zed: spawn shell login mode → capture env JSON → inject vào terminal. OneTerm: inherit env trực tiếp. Shell integration script inject thay vì capture env. |
...
| ✅/❌ | Gap | Zed có | OneTerm có | Mô tả / Ví dụ |
|---|---|---|---|---|
| ❌ | **Task system** | Có — `TaskState`, `task: spawn`, `task: rerun` | Không | Zed: define task trong `tasks.json` → Cmd+Shift+T → spawn. OneTerm: không. **Ví dụ**: Task `cargo test` → Zed spawn terminal, auto-run, show status. |
...
| ❌ | **Multiple terminal tabs** | Có — `Pane` quản lý nhiều `TerminalView` | Không — 1 session/panel | Zed: tạo nhiều terminal, mỗi cái 1 tab trong pane. OneTerm: 1 terminal duy nhất. **Ví dụ**: Zed: "+" button → new terminal tab. OneTerm: không. |
| ❌ | **Terminal rename** | Có — `RenameTerminal` action, inline `Editor` | Không | Zed: right-click tab → Rename → gõ tên. OneTerm: không. |
| ❌ | **Terminal persistence** | Có — `TerminalDb`, `SerializableItem`, `WorkspaceId` | Không | Zed: restore terminal tabs khi reopen workspace. OneTerm: terminal mất khi close. |
| ❌ | **New terminal button in tab bar** | Có — `NewTerminal`, `NewCenterTerminal` buttons | Không | Zed: tab bar có nút "+" tạo terminal mới. |
| ❌ | **Block below cursor (inline blocks)** | Có — `BlockProperties { height, render }` | Không | Zed: Agent panel chèn block UI dưới cursor (vd inline prompt). OneTerm: không. **Ví dụ**: Agent → block 3 dòng "Press Enter to continue" cố định dưới cursor. |
| ❌ | **Embedded mode** | Có — `TerminalMode::Embedded { max_lines_when_unfocused }` | Không | Zed: terminal inline trong editor (Agent panel output). OneTerm: chỉ standalone. |
...
| ❌ | **Font config (family/size)** | Có — `font_family`, `font_size`, `font_features` | Một phần — inherit từ theme | Zed: `"terminal": { "font_family": "JetBrains Mono" }`. OneTerm: dùng theme mono font. |
| ❌ | **Scrollback history config** | Có — `scrollback_history` setting | Hardcoded — 10,000 lines | Zed: `"scrollback_history": 50000`. OneTerm: cố định 10,000. |
...
| ❌ | **Option as Meta** | Có — `option_as_meta: bool` | Không | Zed: macOS → Option key = Meta (Alt). OneTerm: không config. |
| ❌ | **Custom shell arguments** | Có — `with_arguments: { program, args }` | Có (trong config) | Cả hai hỗ trợ args. ✅ Đã có. |
| ❌ | **Environment variables injection** | Có — `env: { KEY: value }` | Không | Zed: `"env": { "MY_VAR": "value" }` → inject vào terminal. OneTerm: inherit env. |
...
| ❌ | **SSH terminal** | Có — remote terminal qua SSH | Thiết kế, chưa impl | Zed: `is_remote_terminal: bool`, remote PTY qua SSH. OneTerm: `SshSession` đã thiết kế (`docs/terminal-backend.md`) nhưng chưa implement. |
| ❌ | **TerminalBuilder (2-step init)** | Có — `TerminalBuilder::new()` → check → `subscribe()` | Không — `LocalSession::spawn` 1 bước | Zed: tách init để handle failure gracefully. OneTerm: `expect("spawn")` → panic nếu fail. **Ví dụ**: PTY fail → Zed show error view. OneTerm: crash. |
...
| ❌ | **Event coalescing (VecDeque)** | Có — `events: VecDeque<InternalEvent>` | Một phần — drain Output events | Zed: queue `InternalEvent` coalesce. OneTerm: drain channel trong spawn task. |
...
| ❌ | **Hyperlink search caching** | Có — `last_hyperlink_search_position`, `hyperlink_regex_searches: RegexSearches` | Không | Zed: cache regex search results để hover nhanh. OneTerm: search mỗi frame. |
