//! Replay a corpus recording through `oneterm-vt`.
//!
//! It produces the two expectation forms the engine being replaced produced when
//! it blessed the corpus at `US-0072`, so the frozen files never move: the
//! engine is measured against them, and it never blesses (R-58). Since `US-0087`
//! nothing can bless at all — the fork that wrote these files is gone, and this
//! module is the only replay left.
//!
//! The encoding therefore speaks the reference's vocabulary in three places
//! where the engine's model is deliberately different:
//!
...
/// The reference's `Flags` bit names, in wire order. Reproduced here rather than
/// read off the fork, which is why the encoding survived its deletion
/// (`US-0087`).
...
fn encode_flags(bits: u16) -> String {
    if bits == 0 {
        return "-".to_owned();
    }
...
/// Replay `recording` through the engine and produce both expectation forms.
pub fn replay(recording: &Recording) -> (GridExpect, StateExpect) {
