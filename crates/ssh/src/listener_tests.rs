...
#[test]
fn reliable_ssh_events_do_not_block_while_term_lock_is_held() {
...
#[test]
fn deferred_ssh_events_flush_in_order_once_the_queue_drains() {
    let (listener, events, _commands) = make_listener(1, 1);
    events.try_send(SessionEvent::Output).unwrap();
    listener.forward(SessionEvent::Bell);
    ...
    let flusher = {
        ...
        std::thread::spawn(move || {
            ...
            listener.flush_reliable_blocking();
            finished_tx.send(()).unwrap();
        })
    };
...
#[test]
fn deferred_ssh_events_keep_fifo_order() {
    let (listener, events, _commands) = make_listener(2, 1);
    events.try_send(SessionEvent::Output).unwrap();
    events.try_send(SessionEvent::Output).unwrap();
    listener.forward(SessionEvent::Bell);
    assert_eq!(events.try_recv().unwrap(), SessionEvent::Output);
