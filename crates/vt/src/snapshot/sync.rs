...
///
/// Owned by the terminal, which hands it to the snapshot state; read it with
/// [`Terminal::sync`](crate::Terminal::sync).
...
impl SyncState {
    /// A closed state: no update open and no watchdog running.
