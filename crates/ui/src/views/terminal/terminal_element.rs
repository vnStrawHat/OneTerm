...
use gpui::{
    fill, point, px, relative, size, App, Bounds, ContentMask, Element, ElementId,
    Entity, FontStyle, Font, FontWeight, GlobalElementId, Hitbox, Hsla, IntoElement,
    LayoutId, Pixels, Point as GpuiPoint, SharedString, TextAlign, TextRun,
...
use super::theme::{ensure_minimum_contrast, resolve_cell_color, TerminalTheme};
...
impl TerminalElement {
            ...
            for ic in line {
                ...
                if !is_default_background_color(&cell.bg) || cell.flags.contains(Flags::INVERSE) {
                    ...
                    rects.push(LayoutRect {
                        point: LayoutPoint { line: display_line, column: col },
                        num_cells: 1,
                        color: bg,
                    });
                }
                ...
                let lp = LayoutPoint { line: display_line, column: point.column.0 as i32 };
    ...
    fn cell_style(cell: &Cell, fg: Hsla, base_font: &Font) -> TextRun {
        let underline = (cell.flags.intersects(Flags::ALL_UNDERLINES) || cell.hyperlink().is_some())
            .then(|| UnderlineStyle { color: Some(fg), thickness: px(1.0), wavy: cell.flags.contains(Flags::UNDERCURL) });
        let strikethrough = cell
            .flags
            .contains(Flags::STRIKEOUT)
            .then(|| gpui::StrikethroughStyle { color: Some(fg), thickness: px(1.0) });
        ...
        TextRun {
            ...
            font: Font { weight, style, ..base_font.clone() },
            underline,
            strikethrough,
        }
    }
...
impl BatchedTextRun {
    fn new(start: LayoutPoint, c: char, mut style: TextRun) -> Self {
        ...
        Self { start, text, cell_count: 1, style }
    }
    ...
    fn paint(&self, origin: GpuiPoint<Pixels>, cell_w: Pixels, line_h: Pixels, font_size: Pixels, window: &mut Window, cx: &mut App) {
        let pos = point(
...
impl Element for TerminalElement {
    ...
    fn prepaint(
        ...
        let cell_width = cx.text_system().advance(font_id, font_px, 'm').map(|s| s.width).unwrap_or(px(8.0));
