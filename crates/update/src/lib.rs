...
impl UpdateManager {
    ...
    pub fn refresh_now(&mut self) -> Result<UpdateCheckResult> {
        self.check_with_cache(false)
    }

    fn check_with_cache(&mut self, use_cached_etag: bool) -> Result<UpdateCheckResult> {
        ...
        let reuse_cached_etag =
            use_cached_etag && self.config.should_reuse_cached_etag(&current_version);
        if use_cached_etag {
        ...
        } else if self.config.last_etag.is_some() {
            log::info!(
        ...
        let Some(releases) = response.releases else {
            let cached_candidate = if use_cached_etag {
