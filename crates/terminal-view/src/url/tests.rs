...
use oneterm_terminal::LineRangeCells;
...
fn make_cells(text: &str, num_cols: usize) -> LineRangeCells {
    grid(text, num_cols, &[]).into_line_range_cells()
}

fn wrapped_cells(text: &str, num_cols: usize, wraps: &[usize]) -> LineRangeCells {
    grid(text, num_cols, wraps).into_line_range_cells()
}
...
fn make_osc8_cells(display: &str, target: &str) -> LineRangeCells {
    let n = display.chars().count().max(1);
    grid(display, n, &[])
        .hyperlink_run(0, 0, n, target)
        .into_line_range_cells()
}
...
fn detect_https_url() {
    ...
    let url = detect_url_at(&cells, 0, 10).expect("url");
...
fn detect_http_url_at_start() {
    ...
    let url = detect_url_at(&cells, 0, 0).expect("url");
...
fn detect_www_url_adds_https() {
    ...
    let url = detect_url_at(&cells, 0, 5).expect("url");
...
fn no_url_in_plain_text() {
    ...
    assert!(detect_url_at(&cells, 0, 5).is_none());
}

#[test]
fn no_url_on_whitespace() {
    let cells2 = make_cells("text https://x.com ", 20);
    assert!(detect_url_at(&cells2, 0, 19).is_none());
}
...
fn phase1_osc8_targets_are_policy_validated() {
    ...
    let detected = detect_url_at(&cells, 0, 0).expect("link");
    assert_eq!(detected.display_text.as_deref(), Some("click me"));
    assert_eq!(
        (detected.start_col, detected.end_col),
        (0, cells.cells.len())
    );
    assert!(matches!(
    ...
    let detected = detect_url_at(&cells, 0, 3).expect("link");
    assert!(matches!(
        validate_target(&detected.url),
        TargetDecision::Deny(_)
    ));
...
fn detect_wrapped_url_click_first_line() {
    ...
    let url = detect_url_at(&cells, 0, 2).expect("url");
...
fn detect_wrapped_url_click_second_line() {
    let cells = wrapped_cells("https://x.com/path", 10, &[0]);
    let url = detect_url_at(&cells, 1, 3).expect("url");
    assert_eq!(url.url, "https://x.com/path");
}
...
fn detect_wrapped_url_three_lines() {
    ...
    let url = detect_url_at(&cells, 2, 0).expect("url");
