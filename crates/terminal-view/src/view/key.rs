...
use oneterm_terminal::{KeyMods, KeySpec, NamedKey};
...
mod tests {
    ...
    fn unknown_named_key_without_modifiers_returns_none() {
        ...
        assert!(map_key(&ks("f25", None, Modifiers::default())).is_none());
    }

    #[test]
    fn plain_a_maps_to_character() {
        let (spec, _) = map_key(&ks("a", Some("a"), Modifiers::default())).unwrap();
